...
export interface DashSection {
  ...
  icon?: string;
  color?: string;
  /** Optional section kind for special rendering (e.g. 'security' shows as status chips). */
  kind?: 'security' | string;
  entities: RoomEntity[];
...
export interface DashView {
  ...
  kind?: 'tiles' | 'cameras' | 'sensors' | 'media' | 'vacuum' | 'climate' | 'robot-v2' | 'automations';
